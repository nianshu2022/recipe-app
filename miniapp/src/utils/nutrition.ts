...
function estimateGrams(amount: number, unit: string): number {
  const factor = unitToGrams[unit] ?? 50
...
export function getCalorieLevel(cal: number): { label: string; level: 'low' | 'medium' | 'high' } {
  if (cal < 300) return { label: '低卡', level: 'low' }
  if (cal <= 600) return { label: '适中', level: 'medium' }
  return { label: '高卡', level: 'high' }
}
